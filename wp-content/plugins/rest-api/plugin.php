...
add_action( 'rest_api_init', 'get_post_category_by_id', 0 );
add_action( 'rest_api_init', 'get_post_image_by_id', 0 );
function get_post_image_by_id(){

    register_rest_field( 'post','post_thumb',  array(
        'get_callback' => 'get_post_post_name_by_id',
...
function get_post_category_by_id(){

    register_rest_field( 'posts','category_name',  array(
        'get_callback' => 'get_post_category_name_by_id',
         'update_callback' => null,
         'schema' => null,
...
function  get_post_category_image_by_id(
)
{
	
	return  get_the_post_thumbnail_url($object['id']);
}
function  get_post_category_name_by_id(
)
{
	//return get_the_category($object['id']);

	$all_category= get_the_category($object['id']);
	$all_category=array( );
	foreach ($all_category as $data) {
		array_push($all_category, array("cat_id"=>$data->cat_ID,"cat_name"=>$data->cat_name));
		# code...
	}
	return $all_category;
}
