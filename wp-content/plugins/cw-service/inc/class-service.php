...
class CW_Service {
	...
	public function __construct() {
		...
		// Handle post columns
		add_filter( 'manage_service_posts_columns', array( $this, 'register_custom_columns' ) );
		add_action( 'manage_service_posts_custom_column', array( $this, 'manage_custom_columns' ), 10, 2 );
	...
	public function register_post_type() {
		...
		$service_permalink = empty( $permalinks['service_base'] ) ? _x( 'service', 'slug', 'cw-service' ) : $permalinks['service_base'];
		$service_page_id   = get_option( $this->option . '_page_id' );

		$labels = array(
			'name'               => _x( 'Services', 'Post Type General Name', 'cw-service' ),
			'singular_name'      => _x( 'Services', 'Post Type Singular Name', 'cw-service' ),
			'menu_name'          => __( 'Services', 'cw-service' ),
			'parent_item_colon'  => __( 'Parent Services', 'cw-service' ),
			'all_items'          => __( 'All Services', 'cw-service' ),
			'view_item'          => __( 'View Services', 'cw-service' ),
			'add_new_item'       => __( 'Add New Service', 'cw-service' ),
			'add_new'            => __( 'Add New', 'cw-service' ),
			'edit_item'          => __( 'Edit Service', 'cw-service' ),
			'update_item'        => __( 'Update Service', 'cw-service' ),
			'search_items'       => __( 'Search Services', 'cw-service' ),
			'not_found'          => __( 'Not found', 'cw-service' ),
			'not_found_in_trash' => __( 'Not found in Trash', 'cw-service' ),
		);
		$args   = array(
			'label'               => __( 'Services', 'cw-service' ),
			'description'         => __( 'Create and manage all services', 'cw-service' ),
			'labels'              => $labels,
	...
	public function register_taxonomy() {
		$permalinks          = get_option( $this->option . '_permalinks' );
		$service_category_base = empty( $permalinks['service_category_base'] ) ? _x( 'service-cat', 'slug', 'cw-service' ) : $permalinks['service_category_base'];


		$labels = array(
			'name'              => __( 'Categories', 'cw-service' ),
			'singular_name'     => __( 'Category', 'cw-service' ),
			'search_items'      => __( 'Search Category', 'cw-service' ),
			'all_items'         => __( 'All Categories', 'cw-service' ),
			'parent_item'       => __( 'Parent Category', 'cw-service' ),
			'parent_item_colon' => __( 'Parent Category:', 'cw-service' ),
			'edit_item'         => __( 'Edit Category', 'cw-service' ),
			'update_item'       => __( 'Update Category', 'cw-service' ),
			'add_new_item'      => __( 'Add New Category', 'cw-service' ),
			'new_item_name'     => __( 'New Category Name', 'cw-service' ),
			'menu_name'         => _x( 'Categories', 'Category Taxonomy Menu', 'cw-service' ),
		);
	...
	public function settings_api_init() {
		...
		add_settings_section(
			...
			'<span id="service-options">' . esc_html__( 'Service', 'cw-service' ) . '</span>',
		...
		add_settings_field(
			...
			'<span class="service-options">' . esc_html__( 'Service page', 'cw-service' ) . '</span>',
		...
		add_settings_section(
			'creative_service_section',
			'<span id="service-options">' . esc_html__( 'Service Item Permalink', 'cw-service' ) . '</span>',
		...
		add_settings_field(
			...
			'<label for="service_category_slug">' . esc_html__( 'Service category base', 'cw-service' ) . '</label>',
	...
	public function reading_section_html() {
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control custom post type content', 'cw-service' ); ?>
		</p>
		<?php
	}
	...
	public function page_field_html() {
		wp_dropdown_pages( array(
			...
			'show_option_none'  => esc_html__( '&mdash; Select &mdash;', 'cw-service' ),
			'option_none_value' => 0,
		) );
	}
	...
	public function service_category_slug_field_html() {
		...
		<input name="service_category_slug" id="service_category_slug" type="text"
		       ...
		       placeholder="<?php echo esc_attr( _x( 'service-cat', 'Service category base', 'cw-service' ) ) ?>"
	...
	public function permalink_section_html() {
		...
		$service_page_id = get_option( $this->option . '_page_id' );
		$base_slug         = urldecode( ( $service_page_id > 0 && get_post( $service_page_id ) ) ? get_page_uri( $service_page_id ) : _x( 'service', 'Default slug', 'cw-service' ) );
		$service_base    = _x( 'service', 'Default slug', 'cw-service' );
		...
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control the permalink used specifically for service.', 'cw-service' ); ?>
		</p>

		<table class="form-table cw-service-permalink-structure">
			<tbody>
			<tr>
				<th>
					<label><input name="service_permalink" type="radio"
					              ...
					              class="cw-service-base" /> <?php esc_html_e( 'Default', 'cw-service' ); ?>
					</label>
				</th>
				<td>
				...
				<tr>
					<th>
						<label><input name="service_permalink" type="radio"
						              ...
						              class="cw-service-base" /> <?php esc_html_e( 'Service base', 'cw-service' ); ?>
						</label>
					</th>
					<td>
			...
			<tr>
				<th>
					<label><input name="service_permalink" type="radio"
					              ...
					              class="cw-service-base" /> <?php esc_html_e( 'Service base with category', 'cw-service' ); ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label><input name="service_permalink" id="cw_service_custom_selection" type="radio"
					              value="custom" <?php checked( in_array( $service_permalink, $structures ), false ); ?> /> <?php esc_html_e( 'Custom Base', 'cw-service' ); ?>
					</label>
				</th>
				<td>
		...
		<script type="text/javascript">
			jQuery( function () {
				jQuery( 'input.cw-service-base' ).change( function () {
					jQuery( '#cw_service_permalink_structure' ).val( jQuery( this ).val() );
				} );
				jQuery( '.permalink-structure input' ).change( function () {
					jQuery( '.cw-service-permalink-structure' ).find( 'code.non-default-example, code.default-example' ).hide();
					if ( jQuery( this ).val() ) {
						jQuery( '.cw-service-permalink-structure code.non-default-example' ).show();
						jQuery( '.cw-service-permalink-structure input' ).removeAttr( 'disabled' );
					} else {
						jQuery( '.cw-service-permalink-structure code.default-example' ).show();
						jQuery( '.cw-service-permalink-structure input:eq(0)' ).click();
						jQuery( '.cw-service-permalink-structure input' ).attr( 'disabled', 'disabled' );
					}
				} );
		...
		if ( isset( $_POST['service_permalink'] ) ) {
			...
			if ( 'custom' === $service_permalink ) {
				...
				if ( '%service_category%' == $service_permalink ) {
					$service_permalink = '/' . _x( 'service', 'slug', 'cw-service' ) . '/' . $service_permalink;
				}
			...
			$service_page_id   = get_option( $this->option . '_page_id' );
			$service_permalink = ( $service_page_id > 0 && get_post( $service_page_id ) ) ? get_page_uri( $service_page_id ) : _x( 'service', 'Default slug', 'cw-service' );
	...
	public function service_post_type_link( $permalink, $post ) {
		...
		if ( ! empty( $terms ) ) {
			...
			$service_category = _x( 'uncategorized', 'slug', 'cw-service' );
		}
